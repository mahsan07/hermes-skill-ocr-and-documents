...
This package expresses **OCR and Documents** as a typed, task-adaptive agent harness. Its concrete capability remains: Extract text and structure from PDFs and scans using OCR and document parsers. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in OCR and Documents |
| --- | --- | --- |
| Memory | `stage_artifact_log` | Retain pdf or scanned images plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `linear_gated_plan` | Transform page rasterizer into the ordered, bounded stages defined by this skill. |
...
| Action | `inspect_preview_authorize_execute_reread` | Advance the workflow through ocr and layout detector while preserving stop conditions and user authority. |
